...
init() ->
	{ok, WatchId} = boss_news:watch("comments",
	fun(created, NewComment) ->
		boss_mq:push(NewComment:get("topic_id"), NewComment)
		% (deleted, OldComment) ->
		% boss_mq:push(OldComment:get("topi_id"), OldComment)
	end),
	{ok, [WatchId]}
. 
